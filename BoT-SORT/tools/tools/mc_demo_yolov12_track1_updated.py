...
def process_folder(folder_path, model, tracker, device, half, imgsz, opt):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        if online_targets:
            ...
            tlbr = best_target.tlbr
            plot_one_box(tlbr, im0s, label=f'{best_target.track_id}', color=[random.randint(0, 255) for _ in range(3)], line_thickness=2)
            ...
            if last_pred_box is not None:
                # Use last known box if no detection is found
                boxes_per_frame.append(last_pred_box.tolist() if hasattr(last_pred_box, 'tolist') else last_pred_box)
